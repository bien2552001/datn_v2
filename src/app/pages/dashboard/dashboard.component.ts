...
export class DashboardComponent implements OnInit {
  ...
  async DTSU666_ui_moment() {
    await this.http.DTSU666_uimoment().subscribe(cs => {
        this.dtsu_ngay = cs;
      });
  }
  async DTSU666_pqphi_moment() {
    await this.http.DTSU666_pqphimoment().subscribe(cs => {
        this.dtsu1_ngay = cs;
      });
  }
  async DTSU666_a_moment() {
    await this.http.DTSU666_amoment().subscribe(cs => {
        this.dtsu2_ngay = cs;
      });
  }
  //PZEM017 
  async Pzem017_uipa_moment() {
    await this.http.Pzem017_uipamoment().subscribe(da => {
